...
| [`backend/`](backend/) | La API HTTP local que la interfaz consume — cinco endpoints, documentados en [`backend/API.md`](backend/API.md); además barre cada minuto los clones que ha atendido y cosecha con `dispatch-check --collect` lo que dejó cada slice cuya PR ya se mergeó | No |
