...
- `reopen` on `blocked-global` opens the phase `fix` at `implement`. The brief carries the
  instruction and the global log.
- The commit of the fix round counts as a slice commit. The run then goes to `global` and on
  green to `slice-judge`.
- `reopen` on an open run, or on a closure outside `blocked-judge`, `blocked-controls` and
  `blocked-global`, exits 9 as today.
- The judge's reopen gives the same run file, brief, prose and announcement as today.
...
| The fix round | the new phase `PHASES.FIX = 'fix'`: steps `implement`, `controls`, `commit`, no judge; its commit opens `global` in the phase `slice`, counters at zero |
...
- The refusal announcement field `failure: { command, code, log }`.
...
// judgesBeforeCommit: case PHASES.FIX → false
...
`'green controls in the fix round go straight to commit'`,
...
`'the run file keeps no slice judge, e2e, reconcile or commit closure'`. Removed on purpose: none.
...
`failure: { command, code, log }`, after `verdict`.
...
`'a refusal prints the failing command its exit and the log after the verdict'`. Removed on purpose: none.
...
class FixRound   // JudgedUnit.of: case PHASES.FIX → new FixRound({ run, tasks, issue })
  stem = 'fix'; get diffBase() → [run.baseSha]; get commands() → every task's commands
  get promisedTests() → PromisedTests.NONE; get commitsForTheSlice() → true
  get verdictPath() → `docs/superpowers/verdicts/issue-<n>-fix.json`
  ...
  get sentBackLines() → []; get vetoedName() → 'the fix round'; get reopenedAt() → 'the fix round'
...
`'the fix round brief leads with the first task and appends the others'`. Removed on purpose: none.
...
export class ReopenedBrief {
  ...
  static section({ closure, instruction, logPath, logText })   // markdown string
}
```
...
- `blocked-global`: `The Global verification of the slice stayed red, or could not be measured,
  and the run was closed. A person opened this fix round: fix what the log shows, in any file of
  the slice. The program runs the Global verification again after your commit. Their instruction:`

Then the instruction, then ``The last log, at `<logPath>` (its last 200 lines):`` and the tail
in a fenced `text` block. With `logText` null, it writes `The log could not be read.` instead.
...
`'an unreadable log is said, not left out'`, `'a closure with no reopen has no brief'`. Removed on purpose: none.
...
keeps `findings` and `verdict`; the other two add `failure: RunClosure.failureOf(run)`. Any other
...
`writeBrief` and `writeJudgeBrief` append `ReopenedBrief.section` when the run carries `reopenedFrom`.
They pass `run.lastFailure.log` and its text, or null when the read throws. Else they append
...
2. The fix round has no judge of its own. D-3 sends its commit to the Global verification and
   then to the slice judge, and the slice judge reads the whole diff. Provenance: issue, D-3.
...
4. The judge's reopen writes no `reopenedFrom`, so its run file stays as today. A run with no
   `reopenedFrom` gets today's advice section. Provenance: issue, acceptance criterion 7.
