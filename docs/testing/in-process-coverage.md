...
The latest main integration preserves #421's separate restart-recovery tests and shared process fixture. The run-driver fixture now recognizes exactly the open/closed GraphQL argument vectors observed in that failed CI run, using the paginated response envelope established by `plugin/__tests__/fixtures/fake-gh-bin/gh`; unknown requests still throw. Typecheck and all 33 selected API, real driver and recovery cases pass with this correction.

Main's subsequent #422 skill-prefix rename is applied to the relocated command implementation and source checks. A path-only capture migration updates request keys/cut points and retains every captured response and file effect byte-for-byte. Focused verification against the renamed skills passed **987/987 tests**, including the 199 original decisions and new actual command/Git checks.
