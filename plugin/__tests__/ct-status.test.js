...
const gitIn = (repo, ...args) => execFileSync('git', ['-C', repo, '-c', 'user.email=t@t', '-c', 'user.name=t', ...args], { stdio: ['ignore', 'ignore', 'pipe'] })

// bench: each test takes its own git checkout, with its `origin` and its
...
function bench({ worktrees = [], origin = 'https://github.com/o/r.git', conCommit = false } = {}) {
  const dir = mkdtempSync(join(tmpdir(), 'ct-st-'))
  const repo = join(dir, 'repo')
  mkdirSync(repo)
  execFileSync('git', ['-C', repo, 'init', '-q'], { stdio: ['ignore', 'ignore', 'pipe'] })
  if (origin) gitIn(repo, 'remote', 'add', 'origin', origin)
  if (conCommit) gitIn(repo, 'commit', '--allow-empty', '-q', '-m', 'base')
...
const run = (b, env = {}, args = ['--repo', 'o/r']) => spawnSync('node', [script, ...args], {
  encoding: 'utf8',
  cwd: b.cwd,
...
const NO_ISSUES = JSON.stringify([[], []])
const openIssue = (n, status, title = 'refresh') => ({ number: n, title: `#${n} ${title}`, body: '', labels: [{ name: `status:${status}` }] })
const inProgress7 = () => JSON.stringify([[openIssue(7, 'in-progress')], []])
const ago = (ms) => JSON.stringify([{ event: 'labeled', label: { name: 'status:in-progress' }, created_at: new Date(Date.now() - ms).toISOString() }])
...
describe('/ct-status', () => {
  ...
  it('.worktrees/ cannot be listed → never "there are no orphans", and the signal comes out `?` instead of `✗`', () => {
    ...
    // `.worktrees/` unreadable, the EACCES `warning:` and a `worktree ✗` came
  ...
  it('with no `ps` on the PATH nobody comes out dead: exit 1 and "it could not be checked"', () => {
    ...
    const res = spawnSync(process.execPath, [script, '--repo', 'o/r'], {
      ...
      env: {
        ...
        FAKE_GH_LIST_SEQUENCE: inProgress7(),
        FAKE_GH_TIMELINE_JSON: ago(3 * 3600_000),
      },
    })
    expect(res.status).toBe(1)
  ...
  it('it mutates nothing: it never calls `gh issue edit` nor `gh label`', () => {
    const b = bench({ worktrees: [7] })
    run(b, { FAKE_GH_LIST_SEQUENCE: inProgress7(), FAKE_GH_TIMELINE_JSON: ago(3 * 3600_000) })
    const log = argvOf(b)
  ...
  it('no read carries --limit, and they all paginate', () => {
    const b = bench()
    run(b, { FAKE_GH_LIST_SEQUENCE: inProgress7(), FAKE_GH_TIMELINE_JSON: '[]' })
    const lines = argvOf(b).split('\n').filter(Boolean)
    expect(lines.length).toBe(3) // open, closed, timeline of #7
...
describe("/ct-status — the checkout's identity", () => {
  ...
  it('invoked FROM INSIDE a worktree, it looks at the main checkout', () => {
    ...
    const b = bench({ conCommit: true })
    gitIn(b.repo, 'worktree', 'add', '-q', '-b', 'feat/7', join(b.repo, '.worktrees', '7'))
...
describe('/ct-status — delivered, waiting for the merge', () => {
  it('three in-review with their worktrees: a block of their own, no residue and exit 0', () => {
    const b = bench({ worktrees: [11, 12, 13] })
    const res = run(b, {
      FAKE_GH_LIST_SEQUENCE: JSON.stringify([[openIssue(11, 'in-review', 'refresh de tokens'), openIssue(12, 'in-review', 'marca de ritmo'), openIssue(13, 'in-review', 'pie de plan')], []]),
    })
...
describe('/ct-status — the sign of life in the residue block', () => {
  it('with a live process inside, it is NOT said that there is none', () => {
    ...
    const child = spawn(join(bin, 'claude'), ['30'], { cwd: join(b.repo, '.worktrees', '9'), stdio: 'ignore' })
    try {
      ...
      const processName = execFileSync('ps', ['-o', 'ucomm=', '-p', String(child.pid)], { encoding: 'utf8' }).trim()
      if (process.platform === 'darwin') {
        expect(processName).not.toBe('claude')
      } else {
        expect(processName).toBe('claude')
      }
      const res = run(b, { FAKE_GH_LIST_SEQUENCE: NO_ISSUES })
  ...
  it('if the process check failed, NOTHING is said about processes', () => {
    ...
    const res = spawnSync(process.execPath, [script, '--repo', 'o/r'], {
      encoding: 'utf8',
      cwd: b.cwd,
      env: { ...process.env, PATH: `${fakeGhDir}:${bin}`, FAKE_GH_COUNTER_FILE: b.counter, FAKE_GH_LIST_SEQUENCE: NO_ISSUES },
    })
    expect(res.status).toBe(1)
    expect(res.stdout).toMatch(/\.worktrees\/9/)
    expect(res.stdout).not.toMatch(/no hay ning.n proceso trabajando dentro/)
...
describe('/ct-status — the whole report reaches the other side of the pipe', () => {
  it('a long report captured by a parent is not truncated at 65536 bytes', () => {
    ...
    const closed = Array.from({ length: 2000 }, (_, i) => ({ number: i + 1, body: '', labels: [{ name: 'status:in-review' }], state_reason: 'completed' }))
    const seqFile = join(b.dir, 'seq.json')
    writeFileSync(seqFile, JSON.stringify([[], closed]))
    const res = run(b, { FAKE_GH_LIST_SEQUENCE_FILE: seqFile })
    expect(res.status).toBe(3)
