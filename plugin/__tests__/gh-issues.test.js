...
describe('issuesQueryFor — one query shape, the states as the caller needs them', () => {
  it('pins the states it is given, paginates by endCursor and asks for the fields the loop reads', async () => {
    ...
    expect(query).toMatch(/nodes\{ number url /)
  ...
  it('it flattens pages, lowercases state, keeps stateReason and normalises labels/milestone', async () => {
    ...
    const pages = [
      { data: { repository: { issues: { nodes: [
        { number: 501, url: 'https://github.com/o/r/issues/501', title: '#1 login', body: 'x <!-- ct-order:1 -->', state: 'OPEN', stateReason: null, milestone: { number: 3, title: 'Epic', description: 'reach' }, labels: { nodes: [{ name: 'type:backend' }, { name: 'area:api' }] } },
    ...
    expect(normalizeGraphqlIssues(pages)).toEqual([
      { number: 501, url: 'https://github.com/o/r/issues/501', title: '#1 login', body: 'x <!-- ct-order:1 -->', state: 'open', stateReason: null, milestone: { number: 3, title: 'Epic', description: 'reach' }, labels: [{ name: 'type:backend' }, { name: 'area:api' }] },
