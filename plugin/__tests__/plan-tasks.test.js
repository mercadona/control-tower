...
// ---------------------------------------------------------------------------
// THE JUDGE LINE — a task declares whether the judge stops at it with
// "**Judge:** checkpoint", one line, optional. A task with no such line is
// not a checkpoint; any value other than "checkpoint" is a problem, because
// `--check-plan` has to refuse a plan a program cannot execute as written,
// not guess what the author meant.
// ---------------------------------------------------------------------------
describe('the plan names its checkpoints', () => {
  const taskWith = (n, name, judgeLine) => [
    `### Task ${n} — ${name}`,
    ...(judgeLine ? [judgeLine] : []),
    '**Tests:** N/A — fixture.',
    '**Verification:**',
  ...
  it('a task that declares **Judge:** checkpoint is a checkpoint, and a task that declares nothing is not', () => {
    ...
    expect(tasks[1].checkpoint).toBe(false)
  })
  ...
  it('any other value of **Judge:** is a problem that names the task', () => {
    ...
    expect(judgeProblems[0].task).toBe(1)
    expect(judgeProblems[0].detail).toContain('**Judge:** always')
    expect(judgeProblems[0].detail).toContain('checkpoint')
  })
})
