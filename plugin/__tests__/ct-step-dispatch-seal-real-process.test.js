...
let repo
const { ct, writeReport, writeVerdict, writeRaw, commits, runState, judgeTask, taskOk } = makeHelpers(() => repo)
...
describe('what next writes down when it prepares a step', () => {
  ...
  it('next_on_judge_seals_its_own_step_so_the_seal_of_implement_does_not_authorise_the_judge', () => {
    Conducting.bothTasksCommitted()

    ct('next')

    expect(runState().nextSeal).toBe('2:judge:1')
  })
