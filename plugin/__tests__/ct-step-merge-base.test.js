...
class RepoMother {
  static #aBareRemoteWithACloneSoOriginMainReallyExists(prefix, issue) {
  ...
  static #advanceTheBaseAndMergeItIntoTheSlice(seed, seedGit, workGit) {
  ...
  static #writeRunPausedAtGlobal(work, issue, baseSha) {
  ...
  static #writePlanAndSeed(work, issue) {
  ...
  static aSliceWhoseBaseShaIsTheCutAndMergedAnAdvancedBase(issue) {
    ...
    RepoMother.#writePlanAndSeed(work, issue)
    writeFileSync(join(work, 'work.txt'), 'trabajo\n')
    workGit('add', '-A')
    workGit('commit', '-qm', 'tarea 1')

    RepoMother.#advanceTheBaseAndMergeItIntoTheSlice(seed, seedGit, workGit)
    RepoMother.#writeRunPausedAtGlobal(work, issue, baseSha)
...
describe('the run commit count', () => {
  it('counts_neither_the_plan_commit_that_precedes_the_run_nor_what_the_merge_of_the_base_brought', () => {
    ...
    const count = (...a) => Number(execFileSync('git', ['rev-list', '--count', ...a], { cwd: work, encoding: 'utf8' }).trim())
    try {
      expect(count(`${baseSha}..HEAD`)).toBe(3)
      expect(count('--no-merges', `${cut}..HEAD`, '^origin/main')).toBe(2)
      expect(count('--no-merges', `${baseSha}..HEAD`, '^origin/main')).toBe(1)
    } finally {
      clean(work, seed, remote)
    }
  })
})
