...
export function buildLauncherScript({ sentinelPath, agentCommand, agentBin, issue, worktree }, shQuote) {
  const q = shQuote
  if (!agentBin) throw new Error('buildLauncherScript: agentBin is missing — it is the name checked with `command -v` inside the login shell, and whether ct-next.mjs undoes the worktree and the claim depends on its result')
  return `#!/bin/sh
...
if [ -e ${q(sentinelPath)} ]; then
  printf '%s\\n' 'ct-next: the agent of #${issue} already started (sentinel present); this sourcing relaunches NOTHING.'
else
