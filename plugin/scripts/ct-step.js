...
import { CommandExit, CommandIo } from './command-io.js'
...
import {
  ...
  PACKAGE_SECTIONS,
  readSliceVerdict,
  outcomeOfSliceVerdict,
  sliceVerdictCommitMessage,
  SLICE_JUDGE_TOOLS,
  SLICE_PACKAGE_SECTIONS,
  RECONCILER_TOOLS,
  REVIEW_TOKEN_LABEL,
  reviewToken,
  reviewTokenLine,
...
export class CtStep {
  static run(argv, io) {
    try {
      return CtStep.#execute(argv, io)
    } catch (failure) {
      if (failure instanceof CommandExit) return failure.code
      throw failure
    }
  }

  static #execute(argv, io) {
    const { readFileSync, writeFileSync, appendFileSync, existsSync, mkdirSync, unlinkSync, readdirSync, statSync } =
      new CommandFiles(io.cwd)
    const resolve = (...paths) => resolvePath(io.cwd, ...paths)
    const execFileSync = (bin, args, options) => CommandIo.execFile(io, bin, args, options)
    const PLUGIN_ROOT = io.pluginRoot
    ...
    const out = (msg) => io.out(msg + '\n')
    const err = (msg) => io.err(msg + '\n')
    const die = (msg, code) => {
      err(msg)
      CommandExit.with(code)
    }
...
exits with 9 and says which one it is. The state lives in .agent/run-<issue>.json.`
    ...
    if (!existsSync(join(repoRoot, '.agent', 'SLICE.md'))) {
      die(
        'this is not a slice worktree (.agent/SLICE.md is missing). ct-step drives the inner stretch of a slice.',
        EXIT.PRECONDITION,
      )
    }
    ...
    if (problems.length) {
      for (const p of problems) err(`plan is not executable: ${p.detail}`)
      CommandExit.with(EXIT.PLAN_NOT_EXECUTABLE)
    }
    ...
    if (existsSync(stateFile)) {
      ...
      if (run.closed === RUN_STATES.DELIVERED) {
        if (verb === 'next') {
          out(
            `run delivered: the ${run.tasksTotal} tasks of issue ${issue} are committed with a verdict, the Global verification is green and the slice is judged. No step is left — open the pull request and release with dispatch-check --release.`,
          )
          CommandExit.with(EXIT.OK)
        }
      ...
      if (actual === null) {
        die(
          `git could not count the run's commits: it does not resolve \`${range}\` in this worktree. Delete ${stateFile} if the run belongs to another branch.`,
          EXIT.PRECONDITION,
        )
      }
      ...
      const expected = SLICE_STEPS.includes(run.step) ? run.tasksTotal + (run.sliceCommits || 0) : run.task - 1
      if (actual !== expected) {
        die(
          `the state and git do not count the same: the file expects ${expected} commit(s) (task ${run.task}, step ${run.step}) and in \`${shortRange}\` (merges excluded) there are ${actual}. It does not carry on blind.`,
          EXIT.PRECONDITION,
        )
      }
    } else {
      if ((git(['diff', '--cached', '--name-only']) || '').trim()) {
        die(
          'the index has staged changes and this run is new. ct-step commits the index task by task: empty it (git reset) or commit it yourself.',
          EXIT.PRECONDITION,
        )
      }
    ...
    const CLAUDE_DIRECTORY = io.env.CLAUDE_CONFIG_DIR || join(io.home, '.claude')
    ...
    const CLAUDE_CODE_HOME = io.env.CLAUDE_CONFIG_DIR || io.home
    ...
    function toolUsageMeasures() {
      const adapter = USAGE_ADAPTERS.find((candidate) => candidate.detects(io.env))
      if (!adapter) return ToolUsage.unsupported().measures()
      try {
        return adapter.usageFor(claimedRequests()).measures()
      } catch (e) {
        err(
          `warning: the tool usage could not be measured (${String(e.message).trim()}). This carries on: no transition depends on the measurement.`,
        )
    ...
    function toolAccountEmail() {
      if (!ClaudeCodeUsage.detects(io.env)) return null
    ...
    function measure(step, measures) {
        ...
        metricRow(
          {
            ...
            task: isSliceStep ? null : run.task,
            task_name: isSliceStep ? null : (currentTask()?.name ?? null),
          ...
          { ...measures, ...toolUsageMeasures(), tool_account_email: toolAccountEmail() },
          { now: new Date(io.now()).toISOString() },
        ),
      )
      ...
      for (const destination of [
        metricsPath('ct-step', { configDir: io.env.CLAUDE_CONFIG_DIR, home: io.home }),
        join(repoRoot, METRICS_REL),
      ]) {
        try {
          ...
          err(
            `warning: the telemetry could not be written to ${destination} (${String(e.message).trim()}). This carries on: no transition depends on the measurement.`,
          )
        }
      }
    }
    ...
    const VERB_OF = {
      report: STEPS.IMPLEMENT,
      controls: STEPS.CONTROLS,
      verdict: STEPS.JUDGE,
      advice: STEPS.ADVISE,
      commit: STEPS.COMMIT,
      reconcile: STEPS.RECONCILE,
      global: STEPS.GLOBAL,
      'slice-verdict': STEPS.SLICE_JUDGE,
      e2e: STEPS.E2E,
    }
    function requireStep(v) {
      if (run.step !== VERB_OF[v]) {
        die(
          `"${v}" is not the step that is due: the run is at "${run.step}" (task ${run.task}/${run.tasksTotal}). Ask with "ct-step next".`,
          EXIT.WRONG_STEP,
        )
      }
    }
    ...
    function nextVerb() {
      ...
      switch (run.step) {
        ...
        case STEPS.CONTROLS:
          ...
          if (t.testsAdded.length)
            out(`  and that the tests the task promised exist: ${t.testsAdded.map((n) => `'${n}'`).join(', ')}`)
          out('')
        ...
        case STEPS.SLICE_JUDGE: {
          ...
          out(
            `  - the log of the Global verification, ALREADY green, in case it wants it: ${run.lastGlobalLog ?? '(N/A declared)'}`,
          )
        ...
        case STEPS.E2E:
          ...
          out(
            'The environment for bringing it up is in the "## Cómo se atraviesa este repo (e2e)" section of AGENTS.md ("Levantar" and "Listo cuando" for each journey). If that section is not filled in, the verdict is `no-verificado` with that reason: never red, and never making up how to start it.',
          )
          ...
          out(
            `Write the report to E2E_SCHEMA (scripts/step-contracts.js): every journey carries ${E2E_SCHEMA.properties.runs.items.required.join(' and ')}, and on top of that, according to its verdict:`,
          )
    ...
    function loadCtYardstick() {
      ...
      if (missing.length) {
        die(
          `ct's yardstick cannot be read: ${missing.join(', ')} is missing or empty in ${join(PLUGIN_ROOT, PluginYardstick.DIRECTORY)}. That is an incomplete plugin installation, not a property of this repo: without those documents whoever implements, judges or reconciles measures against nothing, and that is not silently distinguishable from a conforming diff. Reinstall the plugin.`,
          EXIT.PRECONDITION,
        )
      }
      return ctDocs
    ...
    function repoYardstickSection(artifactName) {
      try {
        ...
        err(
          `warning: ${CONVENTIONS_FILE} exists and could not be read (${String(e.message).trim()}): ${artifactName} goes out without the repo's yardstick.`,
        )
        return ''
      }
    }
    ...
    function writeBrief() {
      ...
      try {
        ...
        die(
          `the brief of task ${run.task} could not be extracted: ${String(e.stderr || e.message).trim()}`,
          EXIT.PRECONDITION,
        )
      }
    ...
    function writeSliceReviewPackage() {
      ...
      writeFileSync(
        ...
        [
          ...
          '',
          `## ${YARDSTICK_SECTION}`,
          `Open it with \`Read\`: \`${simplicityPath}\``,
          '',
          `## ${SIGNAL_SECTION}`,
          sliceSignal ?? SIGNAL_ABSENT,
          '',
          `## ${COMMITS_SECTION}`,
          git(['log', '--reverse', '--format=%h %s', `${run.baseSha}..HEAD`]) || '',
          '',
          `## ${FILES_SECTION}`,
          git(['diff', '--stat', run.baseSha, 'HEAD']) || '',
          '',
          `## ${DIFF_SECTION}`,
          diff,
        ].join('\n'),
    ...
    function writeAdviceReviewPackage() {
      ...
      writeFileSync(
        ...
        [
          ...
          '',
          `## ${BRIEF_SECTION}`,
          readOrAbsent(briefPath(), "the task's brief"),
          '',
          `## ${ATTEMPTS_SECTION}`,
          sectionsByAttempt('report', "the implementer's report"),
          '',
          `## ${VERDICTS_SECTION}`,
          sectionsByAttempt('verdict', "the judge's verdict"),
        ].join('\n'),
      )
      return packagePath
    }
    ...
    function sectionsByAttempt(kind, what) {
      ...
      return files
        .map((f) =>
          [`### Attempt ${ARCHIVED_ATTEMPT_RE.exec(f)[1]}`, '', readOrAbsent(join(workDir, f), what)].join('\n'),
        )
        .join('\n\n')
    }
    ...
    function archive(kind, content) {
      try {
        writeFileSync(
          join(workDir, `task-${run.task}-${kind}-${currentAttempt()}.json`),
          JSON.stringify(content, null, 2) + '\n',
        )
      } catch (e) {
        err(
          `warning: ${kind} of attempt ${currentAttempt()} could not be archived (${String(e.message).trim()}): if this task reaches the advisor, its package will say so.`,
        )
      }
    }
    ...
    function currentToken(packagePath, diffNow) {
      ...
      try {
        ...
        return {
          why: `the review package exists and cannot be read (${packagePath}): ${String(e.message).trim()} — go back to "ct-step next", the only step that generates it, and REDISPATCH the judge`,
        }
      }
    ...
    function consumePackage(packagePath) {
      try {
        ...
        err(
          `warning: the verdict was measured but the review package (${packagePath}) could NOT be consumed: ${String(e.message).trim()}. The step carries on, but that file no longer corresponds to any pending trial: go back to "ct-step next" before dispatching the judge again, because a package that outlives its verdict is the one that lets a stale trial through.`,
        )
      }
    }
    ...
    // left orphaned until somebody commits it separately. `isRunArtifact` no longer
    ...
    const treeEntries = () => {
      ...
      return entries.filter(({ path }) => {
        ...
        seen.add(path)
        return isSafePath(path) && !isRunArtifact(path) && (path === planRelPath() || !isMachineryPath(path))
      })
    }
    ...
    function reconcilePackages() {
      try {
        return readdirSync(workDir)
          .filter((f) => /^reconcile-package-\d+\.md$/.test(f))
    ...
    function reportVerb() {
      const { value, why: whyRead } = readJson(argv[1], 'report')
      ...
      if (onlyDeclared.length || onlyMeasured.length) {
        err(
          `warning: what the implementer declares and what the tree says do not match. What is staged is what was MEASURED.${onlyMeasured.length ? ` Touched but not declared: ${onlyMeasured.join(', ')}.` : ''}${onlyDeclared.length ? ` Declared but not touched: ${onlyDeclared.join(', ')}.` : ''}`,
        )
      }
    ...
    function controlsVerb() {
      ...
      measure('controls', {
        ...
        duration_ms: io.now() - startedAt,
      })
      run = { ...run, lastControlsLog: log }
      out(
        `controls: ${result}${skippedForNoCode ? ' (the suite did not run: the diff carries no code, only documentation)' : ''} (log at ${log})`,
      )
      return result
    }
    ...
    const workingPathsInTheIndex = () => stagedPaths().filter((p) => p !== planRelPath() && !isMachineryPath(p))
    ...
    function declaredScope(t) {
      ...
      for (const path of touched) {
        if (!declared.some((f) => f.path === path)) {
          failures.push(
            `task ${t.n} touched '${path}' and the plan does not declare it in its **Files:** — two explanations are equally plausible and this control cannot arbitrate between them: it is surplus in the CODE, or it needs adding to the PLAN`,
          )
        }
      }
      ...
      for (const f of declared) {
        if (!touched.includes(f.path)) {
          failures.push(
            `the plan declares '${f.path}' in the **Files:** of task ${t.n} and it is not among what was touched: write the CODE the task promised. If it really is surplus in the PLAN, REMOVING IT IS NOT YOUR WAY OUT —an amendment can only add paths, because removing them switches this very control off— so say it in your report and leave the path in the plan`,
          )
          continue
        }
        ...
        if (f.action === 'create' && existedBefore) {
          failures.push(
            `the plan declares '${f.path}' as (create) and it already existed in the previous commit — check the PLAN, the action should be (modify)`,
          )
        }
        if (f.action === 'modify' && !existedBefore) {
          failures.push(
            `the plan declares '${f.path}' as (modify) and it did not exist in the previous commit — check the PLAN, the action should be (create)`,
          )
        }
      }
    ...
    function amendmentOnlyAdds(t) {
      ...
      if (previous === null) {
        return [
          `'${path}' could not be read at HEAD: with no committed plan there is nothing to compare the tree's against, and this control cannot measure whether task ${t.n} removed paths from its **Files:**. Commit the plan —the \`plan\` gate already asks for it before implementing— and ask for the step again.`,
        ]
      }
      ...
      if (!previousTask) {
        return [
          `HEAD's plan declares no task ${t.n}, so this control cannot measure whether the amendment removed paths from its **Files:**. An amendment neither adds nor removes TASKS: that throws the run's count out.`,
        ]
      }
      ...
      return previousTask.files
        ...
        .map(
          (f) =>
            `task ${t.n} amended the plan by removing '${f.path}' from its **Files:** — an amendment can only ADD paths: removing one switches the scope control off from inside. Put the path back in the PLAN, or write the CODE it promised.`,
        )
    }
    ...
    function inIndex(name) {
      ...
      try {
        execFileSync('git', ['grep', '--cached', '--quiet', '-F', '-e', name, '--', ...scope], {
          cwd: repoRoot,
        ...
        throw new NameLookupDidNotRun(
          `git grep for '${name}' did not run: exit ${e.status}, ${e.stderr?.toString() ?? ''}`,
        )
      }
    }
    ...
    function declaredTests(t) {
      const failures = []
      for (const n of t.testsAdded)
        if (!inIndex(n)) failures.push(`the task said it was adding the test '${n}' and it is not in what is staged`)
      for (const n of t.testsRemoved)
        if (inIndex(n)) failures.push(`the task said it was removing the test '${n}' and it is still there`)
      return failures
    }
    ...
    function declaredBlocks(t) {
      ...
      for (const { role, path } of t.blockPaths) {
        if (!touched.includes(path)) {
          failures.push(
            `task ${t.n} declares a ${role} block (${path}) and it is not among what was touched — it is missing from the CODE, or the block is surplus in the PLAN`,
          )
        }
      }
      ...
      for (const { path, text } of t.finalTexts) {
        ...
        if (staged === null) {
          failures.push(
            `task ${t.n} declares a Final text (${path}) and that file is not among what was touched — it is missing from the CODE, or the block is surplus in the PLAN`,
          )
          continue
        }
        for (const line of text.split('\n')) {
          if (line.trim() !== '' && !staged.includes(line)) {
            failures.push(
              `task ${t.n} declares Final text (${path}) and the line '${line}' is not verbatim in what is staged — it is missing from the CODE, or the PLAN quotes the text wrongly`,
            )
          }
        }
      }
    ...
    const gitForReconcile = (argv) => {
      try {
        ...
        if (typeof e.status !== 'number') {
          die(
            `git ${argv.join(' ')} did not finish on its own (signal ${e.signal || 'unknown'}): it hit the time cap or somebody killed it. That is not distinguishable from a git failure and it is not going to be read as one.`,
            EXIT.PRECONDITION,
          )
        }
    ...
    function planSection(markdown, heading) {
    ...
    const DISCARD_FIX = {
      [DiscardReason.MARKERS_LEFT]:
        'conflict markers (<<<<<<< / ======= / >>>>>>>) were left in place in one of the resolved files.',
      [DiscardReason.TOUCHED_OUTSIDE_THE_CONFLICT]:
        'the resolution touched files that were not on the conflict list: the index can only carry the files in dispute.',
      [DiscardReason.UNRESOLVED_FILES_REMAIN]:
        'there are still unresolved files after trying to stage them: they all have to be resolved before concluding.',
    }
    ...
    function writeReconcileReviewPackage({ branch, round, attempt }) {
      ...
      const lines = [
        ...
        baseLog(branch),
        '',
        planSection(planText, '### Desired end state'),
      ]
      if (round.reason) {
    ...
    function handoverToSliceAgent() {
      out(
        `ct-reconciler used up its ${DEFAULT_BUDGETS.reconcileRetries} round(s) without resolving it: it is now up to the slice's own agent, which does have Bash. Have it resolve the conflict by hand, leave the files staged and call:`,
      )
      out(`  ct-step reconcile --plan ${planPath} --issue ${issue}`)
    }
    ...
    function reconcileVerb() {
      ...
      if (!branch) {
        die(
          'reconcile cannot resolve the slice\'s base branch (no "base:" in .agent/SLICE.md, no remote main/master in this worktree): there is nothing to merge with.',
          EXIT.PRECONDITION,
        )
      }
      ...
      switch (round.outcome) {
        ...
        case ReconcileOutcome.MARKERS_COMMITTED:
          out(
            `reconcile: markers-committed — HEAD is already a merge commit, made outside this verb, and ${round.files.length} of its file(s) carry conflict markers INSIDE the commit:`,
          )
          for (const f of round.files) out(`  - ${f}`)
          out(
            'There is no live merge to conclude and no round to discard: the pull request would carry the markers inside, and if the conflict falls in a file the controls do not compile, it comes out green.',
          )
          out(
            "DISPATCH THE SLICE'S AGENT (it has Bash) to remove the markers and commit the fix, and ask again with ct-step next.",
          )
          break
        ...
        case ReconcileOutcome.UNMERGEABLE_TREE:
          out(
            `reconcile: unmergeable-tree — git could not start the merge with "${branch}": this is not a content conflict, it is the slice's own tree (uncommitted changes, or something half-finished).`,
          )
        ...
        case ReconcileOutcome.ROUND_DISCARDED: {
          ...
          out(
            'The round was discarded without committing anything: the merge is still live, with the conflicting files restored to how git left them.',
          )
          ...
          const packagePath = writeReconcileReviewPackage({ branch, round, attempt: nextReconcileAttempt() })
          out(
            `REDISPATCH ct-reconciler (subagent — declared WITHOUT Bash and WITHOUT Write: ${RECONCILER_TOOLS}) with the new package:`,
          )
          out(`  - the reconciliation package: ${packagePath}`)
    ...
    function globalVerb() {
      const startedAt = io.now()
      if (!globalVerification.commands.length) {
        measure('global', { outcome: OUTCOMES.DONE, global_log: null, commands: 0, duration_ms: io.now() - startedAt })
      ...
      measure('global', {
        outcome: result,
        global_log: log,
        commands: globalVerification.commands.length,
        duration_ms: io.now() - startedAt,
      })
    ...
    function sliceVerdictVerb() {
      ...
      const { value, why: whyRead } = readJson(argv[1], 'slice verdict')
      ...
      measure('slice-judge', {
        ...
        review_token: token,
        ...verdictMeasures(verdict),
        ...roleMeasures(STEPS.SLICE_JUDGE, packagePath),
      })
      ...
      if (verdict.ruling === 'PASS') {
        ...
        writeFileSync(
          join(repoRoot, path),
          JSON.stringify({ issue, tasks_total: run.tasksTotal, verdict }, null, 2) + '\n',
        )
        ...
        if (git(['add', '--', path], { allowFail: true }) === null) {
          err(
            `warning: the slice's verdict was written to ${path} but could NOT be staged, so it will not travel in the pull request (is the path gitignored in this repo?). The delivery carries on.`,
          )
        }
        if (existsSync(join(repoRoot, METRICS_REL)) && git(['add', '--', METRICS_REL], { allowFail: true }) === null) {
          err(
            `warning: the telemetry (${METRICS_REL}) could not be staged — the slice's verdict travels without it. Is the path gitignored in this repo?`,
          )
        }
        ...
        if (foreign.length) {
          ...
          if (message !== null) {
            if (git(['commit', '-m', message], { allowFail: true }) === null) {
              err(
                "warning: the slice's verdict could not be committed — the delivery does not depend on the evidence, but check the index before opening the pull request.",
              )
            } else {
          ...
          err(
            "warning: nothing to commit of the slice's verdict (are both paths gitignored?) — the delivery carries on.",
          )
        }
      }
    ...
    function verdictVerb() {
      ...
      const { value, why: whyRead } = readJson(argv[1], 'verdict')
      ...
      measure('judge', {
        ...
        review_token: token,
        ...verdictMeasures(verdict),
        ...roleMeasures(STEPS.JUDGE, packagePath),
      })
      ...
      run = {
        ...run,
        lastVerdict: verdict,
        lastFindings: major.length
          ? major.map((f) => `- [${f.severity}] ${findingLocation(f)}: ${f.what}`).join('\n')
          : null,
      }
      if (verdict.ruling === 'PASS') {
        ...
        writeFileSync(
          join(repoRoot, path),
          JSON.stringify({ issue, task: run.task, task_name: currentTask()?.name ?? null, verdict }, null, 2) + '\n',
        )
        ...
        if (git(['add', '--', path], { allowFail: true }) === null) {
          err(
            `warning: the verdict was written to ${path} but could NOT be staged, so it will not travel in the pull request (is the path gitignored in this repo?). The task is committed all the same.`,
          )
        } else {
    ...
    function adviceVerb() {
      ...
      out(
        `advice accepted: ${advice.files_to_reconsider.length} path(s) to reconsider; the tree goes back to the last commit in ${cleaned} path(s)`,
      )
      return OUTCOMES.DONE
    }
  ...
  - if that code HAS to go in, it does not go in through here: from "commit" there is no way back to the judge in this run. Take it out of the index, commit the approved task, and let that work come in through the next task or through another slice.`)
      ...
      if (existsSync(join(repoRoot, METRICS_REL)) && git(['add', '--', METRICS_REL], { allowFail: true }) === null) {
        err(
          `warning: the telemetry (${METRICS_REL}) could not be staged — the task is committed without it. Is the path gitignored in this repo?`,
        )
      }
    ...
    function writeE2eReport(runs) {
      ...
      if (git(['add', '--', path], { allowFail: true }) === null) {
        err(
          `warning: the e2e report was written to ${path} but could NOT be staged, so it will not travel in the pull request (is the path gitignored in this repo?).`,
        )
      }
      return path
    }
    ...
    function e2eVerb() {
      const { value, why: whyRead } = readJson(argv[1], 'e2e report')
      ...
      run = {
        ...
        e2eResults: runs.map((r) =>
          r.reason ? { run: r.run, verdict: r.verdict, reason: r.reason } : { run: r.run, verdict: r.verdict },
        ),
      }
...
Co-Authored-By: Claude <noreply@anthropic.com>`
      ...
      if (keywords.length) {
        err(
          `warning: the commit message of the e2e report contains a closing keyword (${keywords.map((k) => `${k.keyword} ${k.ref}`).join(', ')}) and would close the issue without anybody having decided it — it is NOT committed. The report (${path}) is left staged.`,
        )
        return
      }
      ...
      if (git(['commit', '-m', message], { allowFail: true }) === null) {
        err(
          `warning: the e2e report (${path}) was left staged but could NOT be committed — check it by hand before opening the pull request.`,
        )
        return
      }
    ...
    try {
      ...
      const outcome = {
        report: reportVerb,
        controls: controlsVerb,
        verdict: verdictVerb,
        advice: adviceVerb,
        commit: commitVerb,
        reconcile: reconcileVerb,
        global: globalVerb,
        'slice-verdict': sliceVerdictVerb,
        e2e: e2eVerb,
      }[verb]()
      ...
      if (run.discards >= MAX_DISCARDS && outcome === OUTCOMES.DISCARDED) {
        save()
        die(
          `${run.discards} discards in this run: it stops instead of going on asking for answers that cannot be read`,
          EXIT.NO_VERDICT,
        )
      }
      ...
      CommandExit.with(exitCodeOf(transition.state, before, outcome))
    } catch (e) {
      if (e instanceof CommandExit) throw e
      save()
      err(`unforeseen exception: ${e.stack || e.message}`)
      CommandExit.with(EXIT.UNNAMED)
    }
    ...
    function exitCodeOf(state, step, outcome) {
      switch (state) {
        case RUN_STATES.DELIVERED:
          out(
            'the tasks committed, the Global verification green and the slice with a PASS verdict: the branch is ready for the pull request.',
          )
          return EXIT.OK
        case RUN_STATES.BLOCKED_COMMIT:
        ...
        // anything else.
        case RUN_STATES.BLOCKED_RECONCILE:
          return EXIT.RECONCILE_BLOCKED
        ...
        case RUN_STATES.BLOCKED_SLICE_JUDGE:
          return EXIT.VETOED
        case RUN_STATES.BLOCKED_E2E:
          return EXIT.E2E_RED
        case RUN_STATES.ABORTED_BUDGET:
          return EXIT.UNNAMED // nobody measures money here
        default:
          return EXIT.UNNAMED
      }
    }
