...
describe('Home · sessions column width', () => {
  ...
  it('keeps the work area at least 600px wide, so a maxed-out column never clips the flow bar', async () => {
    mockColumnsWidth(1200)
    openHome()

    await screen.findByRole('navigation', { name: 'Navegación principal' })

    await waitFor(() => expect(screen.getByRole('separator', { name: LABEL })).toHaveAttribute('aria-valuemax', '600'))
  })
