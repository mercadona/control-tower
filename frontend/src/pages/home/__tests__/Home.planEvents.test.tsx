...
describe('Home · plan events', () => {
  ...
  it('keeps implementation current when reopening the completed request summary during planning', async () => {
  ...
  it('should say the plan is being written when the first frame arrives', async () => {
    ...
    expect(within(screen.getByLabelText('Implementación', { selector: 'section' })).getByLabelText('Progreso del plan')).toBeInTheDocument()
  })

  it('keeps a ready plan visible in implementation while waiting for automatic execution', async () => {
    await planStarted()
    ...
    expect(screen.getByRole('navigation', { name: 'Flujo del plan' }).querySelector('[aria-current="step"]')).toHaveTextContent('Implementación')
    expect(screen.queryByText('Revisar plan')).not.toBeInTheDocument()
