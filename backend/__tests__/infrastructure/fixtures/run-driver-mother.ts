...
export class RunDriverMother {
  ...
  async deliverThroughApi(): Promise<{
    ...
    const response = await fetch(`http://127.0.0.1:${port}/start-plan`, {
      ...
      body: JSON.stringify({
        user_comment: 'Run the finite offline driver fixture',
  ...
  async #executables(): Promise<void> {
    ...
    await writeFile(join(this.bin, 'gh'), [
      ...
      "const openQuery = 'query($endCursor:String,$owner:String!,$name:String!){ repository(owner:$owner,name:$name){ issues(first:100,after:$endCursor,states:[OPEN],orderBy:{field:CREATED_AT,direction:ASC}){ nodes{ number title body state stateReason milestone{number title description} labels(first:50){nodes{name}} } pageInfo{ hasNextPage endCursor } } } }'",
      "const listing = (query) => ['api', 'graphql', '--paginate', '--slurp', '-f', 'query=' + query, '-f', 'owner=acme', '-f', 'name=widget']",
      ...
      "else if (equal(listing(openQuery))) console.log(JSON.stringify([{ data: { repository: { issues: { nodes: [{ number: 7, title: 'Finite fixture', body: '<!-- ct-order:1 -->', state: 'OPEN', stateReason: null, milestone: null, labels: { nodes: [{ name: 'status:ready' }] } }], pageInfo: { hasNextPage: false, endCursor: null } } } } }]))",
      "else if (equal(listing(openQuery.replace('states:[OPEN]', 'states:[CLOSED]')))) console.log(JSON.stringify([{ data: { repository: { issues: { nodes: [], pageInfo: { hasNextPage: false, endCursor: null } } } } }]))",
