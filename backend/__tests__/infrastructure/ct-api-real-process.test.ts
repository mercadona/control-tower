...
class Entrypoint {
  ...
  static async startedWithStderr(environment: NodeJS.ProcessEnv): Promise<Started> {
    return await Entrypoint.#started(environment)
  }
...
describe('ct-api entrypoint', () => {
  ...
  it('the runtime sweeps every registered checkout and mounts the slice message path', async () => {
    ...
    try {
      const started = await Entrypoint.startedWithStderr(ADraftSpecCheckout.environment(fixture))
