...
| **GO** | The retired distributed protocol's human `-OK <nonce>` on an issue. No module of this backend mints, writes or reads one any more, and none names a nonce or a go registry: gate 2 authorises the work and successful publication continues automatically. `POST /implement-plan`, the path that answered it, is not mounted and answers 404 |
...
| **External tool** | A binary Control Tower drives that has to be usable before work starts: `gh`, `acli`, `claude`, `git` and `bq`. Which five lives in `probed-tool-sessions.ts`; `gh`, `acli`, `claude`, `git` (through `ssh`) and `bq` (through `gcloud`) are probed |
...
| **Conversation** | The identity of a coordinating session across a restart: an id minted once, a repository, a checkout root and the story it was opened for, recorded on disk so `records.recall()` can find it again; a conversation Claude Code no longer holds answers `unresumable` instead of being silently reopened as a different one |
...
| **Execution spec** | The epic's central document in the governed checkout, `docs/superpowers/specs/<story>-execution.md`, its path derived from the story of the conversation that writes it; gate 1 reads its state line to know whether the epic is frozen, and is what writes it |
